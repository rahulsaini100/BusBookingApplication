...
class SaveUserControllerTest {
	...
	void NumberAlreadyregistered_BadRequest_Test() throws Exception {
		...
		MvcResult mvcResult = mockMvc
				.perform(post("/saveuser?userName=Vikas&userPhoneNumber=8814056123&userAddress=Delhi,NCR")).andReturn();
	...
	void IFuserNameNotContainsAlphabatOrSpacesOnly_BadRequest_Test() throws Exception {
		...
		MvcResult mvcResult = mockMvc
				.perform(post("/saveuser?userName=Vikas123&userPhoneNumber=8814056123&userAddress=Delhi,NCR"))
				.andReturn();
		int status = mvcResult.getResponse().getStatus();
		assertEquals(400, status);
		String message = mvcResult.getResponse().getErrorMessage();
	...
	void IfuserNumberNotContainDigitsOnly_BadRequest_Test() throws Exception {
		...
		MvcResult mvcResult = mockMvc
				.perform(post("/saveuser?userName=Vikas&userPhoneNumber=8814056abc&userAddress=Delhi,NCR")).andReturn();
		int status = mvcResult.getResponse().getStatus();
		assertEquals(400, status);
		String message = mvcResult.getResponse().getErrorMessage();
	...
	void IfuserNumberGreaterThan10Digits_BadRequest_Test() throws Exception {
		...
		MvcResult mvcResult = mockMvc
				.perform(post("/saveuser?userName=Vikas&userPhoneNumber=88140562345&userAddress=Delhi,NCR"))
				.andReturn();
		int status = mvcResult.getResponse().getStatus();
		assertEquals(400, status);
		String message = mvcResult.getResponse().getErrorMessage();
